...
<div class="border-l-2 pl-4 {{ $borderClass }} {{ $isRest ? 'border-dashed' : '' }}">
    {{-- Block header: badge top-right --}}
    <div class="flex items-start justify-between gap-3">
        <flux:heading size="sm">{{ $block->block_type->label() }}</flux:heading>

        <flux:badge color="{{ $block->block_type->color() }}" icon="{{ $block->block_type->icon() }}" size="sm" class="shrink-0">
