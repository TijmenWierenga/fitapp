    ...
    @if($this->completedWorkouts->count() > 0)
        <div class="flex flex-col gap-3 max-h-96 overflow-y-auto">
            @foreach($this->completedWorkouts as $workout)
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 p-3 rounded-lg border border-zinc-200 dark:border-zinc-700">
                    <a href="{{ route('workouts.show', $workout) }}" class="flex-1 min-w-0">
                        ...
                        <flux:text class="text-xs sm:text-sm text-zinc-500 dark:text-zinc-400 mt-0.5 ml-7">
                    ...
                    <flux:badge color="green" size="sm" class="self-start sm:self-auto">
        ...
        @if(auth()->user()->workouts()->completed()->count() > 10)
            <flux:text class="text-xs sm:text-sm text-zinc-500 dark:text-zinc-400 text-center mt-4">
